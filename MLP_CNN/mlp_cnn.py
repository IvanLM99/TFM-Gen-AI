...
def _create_plot(fig_title, plot_data):
    """Helper to generate the 2x2 projection plot."""
    fig = plt.figure(figsize=(15, 14)); fig.suptitle(fig_title, fontsize=18, y=0.98)
    gs_main = GridSpec(2, 2, figure=fig, height_ratios=[0.8, 1]); ax_img1 = fig.add_subplot(gs_main[0, 0]); ax_img2 = fig.add_subplot(gs_main[0, 1])
    gs_bottom = gs_main[1, :].subgridspec(1, 3, width_ratios=[1, 1, 0.05], wspace=0.3)
    ax_ls1 = fig.add_subplot(gs_bottom[0, 0]); ax_ls2 = fig.add_subplot(gs_bottom[0, 1]); colorbar_axis = fig.add_subplot(gs_bottom[0, 2])
    ...
    fig.colorbar(scatter_artist, cax=colorbar_axis, label='Digit Class', ticks=np.arange(10)); fig.tight_layout(rect=[0, 0, 1, 0.94]); return fig
...
def visualize_vae_to_gan(vae_model, gan_generator, regressor, classifier, device, vis_data, test_dataset, seed, num, config_tag, output_dir):
    """Visualizes the VAE -> GAN pipeline, including CNN label prediction."""
    ...
    mse = mean_squared_error(img.squeeze().numpy(), final_img_np)
    plot_data = {
        'img1': img.squeeze(), 'img2': final_img_np,
        'title1': f"1. Real image (True Class: {label})\nEncoded to VAE Vector: ({z_vae_np[0,0]:.2f}, {z_vae_np[0,1]:.2f})",
        'title2': f"4. Final image (Predicted Class: {pred_label})\nFrom GAN Vector: ({pred_gan_vec_np[0,0]:.2f}, {pred_gan_vec_np[0,1]:.2f})",
    ...
    fig = _create_plot(f"VAE -> GAN Projection (Config: {config_tag})\nImage-to-Image MSE = {mse:.4f}", plot_data)
...
def visualize_gan_to_vae(vae_model, gan_generator, regressor, classifier, device, vis_data, test_dataset, seed, num, config_tag, output_dir):
    """Visualizes the GAN -> VAE pipeline, including CNN label prediction."""
    (X_vae, y_vae), (X_gan, y_gan) = vis_data
    np.random.seed(seed); random_idx = np.random.randint(0, len(test_dataset))
    img, label = test_dataset[random_idx]
    ...
    plot_data = {
        ...
        'title2': f"4. Final image (Predicted Class: {pred_label})\nFrom VAE Vector: ({pred_vae_vec_np[0,0]:.2f}, {pred_vae_vec_np[0,1]:.2f})",
    ...
    fig = _create_plot(f"GAN -> VAE Projection (Config: {config_tag})\nImage-to-Image MSE = {mse:.4f}", plot_data)
...
def main():
    ...
    common_data = {
        ...
        "vis_data": (vis_data, test_dataset),
    ...
    for exp in experiments:
        run_experiment(
            exp_name=exp['name'],
            config_tag=exp['tag'],
            output_dir=os.path.join(BASE_OUTPUT_DIR, exp['tag']),
