...
def main():
    ...
    hidden_dim = 400
    latent_dim_vae = 200 
    latent_dim_plot = 2 # Latent dimension for VAE (mean and logvar layers output 2 features for plotting)
    batch_size = 100 
    ...
    class VAE(nn.Module):
        ...
        def reparameterization(self, mean, log_var, current_device): # Corrected var to log_var
    ...
    def train_vae(model_instance, optimizer_instance, num_epochs, current_device, x_dim=784):
        ...
        history = {'loss': []} # Initialize history dictionary to store loss values
    ...
    # --- Image generation (single digits) ---
    print("\n--- Generating sample digits ---")
    def generate_digit(model_instance, mean_val, var_val_or_z1, current_device, digit_size=28): 
        model_instance.eval() 
        with torch.no_grad(): 
            # Assuming mean_val and var_val_or_z1 are direct components for a 2D latent vector z
            z_sample = torch.tensor([[mean_val, var_val_or_z1]], dtype=torch.float).to(current_device)
            x_decoded = model_instance.decode(z_sample)
            digit = x_decoded.detach().cpu().reshape(digit_size, digit_size) 
            
            plt.figure(figsize=(4,4))
            plt.title(f'Generated Digit from z=[{mean_val:.2f}, {var_val_or_z1:.2f}]')
            plt.imshow(digit, cmap='gray')
            plt.axis('off')
            plt.savefig(f'generated_digit_z_{mean_val:.1f}_{var_val_or_z1:.1f}.png')
            print(f"Generated digit plot saved as 'generated_digit_z_{mean_val:.1f}_{var_val_or_z1:.1f}.png'")

    generate_digit(model, 0.0, 1.0, device)
    generate_digit(model, 1.0, 0.0, device)
    generate_digit(model, -1.0, -1.0, device)
    ...
    def plot_latent_space_scatter(model_instance, data_loader, current_device, num_batches=100):
        ...
        plt.title("VAE Latent Space Scatter Plot (Test Set Means)")
        plt.grid(True)
    ...
    def plot_latent_space(model_instance, current_device, scale=1.5, n=25, digit_size=28, figsize=15):
        # Ensure the latent_dim_plot is 2 for this visualization
        ...
        grid_y = np.linspace(-scale, scale, n)[::-1] # Reversed to match typical image coordinates
        ...
        with torch.no_grad(): # No gradients needed for generation
            for i, yi in enumerate(grid_y):
                for j, xi in enumerate(grid_x):
                    ...
                    x_decoded = model_instance.decode(z_sample)
                    # Assuming x_decoded is (batch_size, feature_size)
                    # If batch_size is 1 (as z_sample is [1,2]), then x_decoded[0] is correct
                    digit = x_decoded[0].detach().cpu().reshape(digit_size, digit_size)
                    figure[i * digit_size : (i + 1) * digit_size, j * digit_size : (j + 1) * digit_size] = digit.numpy() # Ensure it's a numpy array
        ...
        sample_range_x = np.round(grid_x, 1) # Use more precision for labels if scale is small
        ...
        plt.xlabel("mean, z [0]")
        plt.ylabel("var, z [1]")
        plt.imshow(figure, cmap="Greys_r") # Greys_r for black background, white digits
    ...
    def plot_tsne(model_instance, data_loader, current_device, num_batches=100, perplexity_val=30):
        ...
        tsne = TSNE(n_components=2, verbose=0, perplexity=perplexity_val, n_iter=300, random_state=42) # Added random_state, reduced verbosity
        ...
        plt.title(f"t-SNE visualization of VAE Latent Space (Test Set Means, Perplexity={perplexity_val})")
        plt.grid(True)
        plt.savefig(f'vae_tsne_latent_space_perplexity{perplexity_val}.png')
        print(f"t-SNE plot saved as 'vae_tsne_latent_space_perplexity{perplexity_val}.png'")
