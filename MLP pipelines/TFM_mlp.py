...
def _create_plot(fig_title, plot_data, num_top_images=2):
    """
    Helper to generate the projection plot, using the robust `constrained_layout`.
    A 3-image layout shows the intermediate reconstruction from the source model.
    """
    # 1. Use `constrained_layout=True` and remove the fig.tight_layout() call later.
    fig = plt.figure(figsize=(16, 12), constrained_layout=True)
    fig.suptitle(fig_title, fontsize=18)

    if num_top_images == 3:
        # Layout for 3 images on top (Original -> Reconstruction -> Final Projection)
        # 2. Remove `hspace` as constrained_layout will manage spacing automatically.
        gs_main = GridSpec(2, 3, figure=fig, height_ratios=[0.6, 1])
        ax_img1 = fig.add_subplot(gs_main[0, 0])
        ax_img_middle = fig.add_subplot(gs_main[0, 1])
        ax_img2 = fig.add_subplot(gs_main[0, 2])

        ax_img1.imshow(plot_data['img1'], cmap='gray')
        ax_img1.set_title(plot_data['title1'], fontsize=12); ax_img1.axis('off')
        ax_img_middle.imshow(plot_data['img_middle'], cmap='gray')
        ax_img_middle.set_title(plot_data['title_middle'], fontsize=12); ax_img_middle.axis('off')
        ax_img2.imshow(plot_data['img2'], cmap='gray')
        ax_img2.set_title(plot_data['title2'], fontsize=12); ax_img2.axis('off')

        gs_bottom_span = gs_main[1, :]
    else: # Fallback to 2-image layout, now properly centered
        gs_main = GridSpec(2, 4, figure=fig, height_ratios=[0.6, 1])
        ax_img1 = fig.add_subplot(gs_main[0, 1])
        ax_img2 = fig.add_subplot(gs_main[0, 2])
    ...
    gs_bottom = gs_bottom_span.subgridspec(1, 2, wspace=0.3)
    ...
    fig.colorbar(scatter_artist, ax=[ax_ls1, ax_ls2], label='Digit Class', ticks=np.arange(10))
    return fig
...
def visualize_vae_to_gan(vae_model, gan_generator, regressor, classifier, device, vis_data, test_dataset, seed, num, config_tag, output_dir):
    ...
    with torch.no_grad():
        ...
        # --- Generate the VAE's reconstruction of the original image ---
...
def visualize_gan_to_vae(vae_model, gan_generator, regressor, classifier, device, vis_data, test_dataset, seed, num, config_tag, output_dir):
    ...
    with torch.no_grad():
        # --- Generate the reconstructed image from the inverted vector ---
